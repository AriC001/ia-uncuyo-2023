...
class VizDoom(gym.Env):
    ...
    def __init__(self, render = False,env_selection = "basic"):
        ...
        if env_selection == "defend_the_line":
            self.game_path = "C:/Users/arico/Documents/UNC/3ro/Inteligencias Artificial/ia-uncuyo-2023-develop/proyecto_final/code/ViZDoom-master/scenarios/new_defend_the_line.cfg"
            # self.game_path = "./defend_the_line.cfg"

        if env_selection == "defend_the_center":
            self.game_path = "./defend_the_center.cfg"
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(160,160,1), dtype=np.uint8)
    ...
    def grayScale(self):
        ...
        cropScreenBuffer = cv2.resize(cropScreenBuffer, (160,160), interpolation = cv2.INTER_CUBIC)
        cropScreenBuffer = np.reshape(cropScreenBuffer, (160, 160,1))
    ...
    def step(self, action):
        ...
        frameSkip = 4 #number of frames to skip
        ...
        return grayScaledImg, reward+self.reward, self.game.is_episode_finished(), truncated,info
    ...
    def get_total_reward(self):
        rw = self.game.get_total_reward() + self.reward
        return rw
